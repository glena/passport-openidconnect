...
Strategy.prototype.authenticate = function(req, options) {
  ...
  if (req.query && req.query.code) {
    ...
    this.configure(null, function(err, config) {
      ...
      function loaded(err, ok, state) {
        ...
        oauth2.getOAuthAccessToken(code, { grant_type: 'authorization_code', redirect_uri: callbackURL }, function(err, accessToken, refreshToken, params) {
          ...
          if (state.meta && state.meta.nonce && (!jwtClaims.nonce || jwtClaims.nonce !== state.meta.nonce)) {
    ...
    this.configure(identifier, function(err, config) {
      ...
      let nonce;
      console.log(typeof self._nonce, (self._nonce && typeof self._nonce === 'boolean'))
      if (self._nonce && typeof self._nonce === 'boolean') { 
        nonce = utils.uid(20); 
      }
      if (self._nonce && typeof self._nonce === 'number') { 
        nonce = utils.uid(self._nonce); 
      }
      if (self._nonce && typeof self._nonce === 'string') { 
        nonce = self._nonce; 
      }
